...
class Lock(object):
    ...
    def __init__(self, redis_client, name, expire=None, id_=None, auto_renewal=False, strict=True, signal_expire=1000):
        """
        :param redis_client:
        ...
        :param signal_expire:
            Advanced option to override signal list expiration in milliseconds. Increase it for very slow clients. Default: ``1000``.  # noqa
        """
        ...
        if id_ is None:
            ...
            try:
                self._id = id_.decode("ascii")
            except UnicodeDecodeError:
                self._id = b64encode(id_).decode("ascii")
        elif isinstance(id_, str):
            self._id = id_
        else:
            raise TypeError(f"Incorrect type for `id`. Must be bytes/str not {type(id_)}.")
    ...
    def extend(self, expire=None):
        ...
        if expire:
            ...
            raise TypeError(
                "To extend a lock 'expire' must be provided as an argument to extend() method or at "
                "initialization time."
            )
    ...
    def release(self):
        ...
        error = self.unlock_script(
            client=self._client, keys=(self._name, self._signal), args=(self._id, self._signal_expire)
        )
        if error == 1:
            raise NotAcquired(f"Lock({self._name}) is not acquired or it already expired.")
        elif error:
